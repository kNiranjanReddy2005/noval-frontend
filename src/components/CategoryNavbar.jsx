...
const CategoryNavbar = ({ activeMenu, onMenuChange }) => {
  const activeCategory = menuData[activeMenu] ?? menuData["Academic Hub"];
  const activeTheme =
    categoryThemes[activeMenu] ?? categoryThemes["Academic Hub"];
    ...
    <div className="mt-6 w-full px-2 md:mt-8 md:px-4">
      <div className="flex gap-3 overflow-x-auto border-b border-slate-200 pb-4 scrollbar-hide md:gap-4 md:pb-5">
        {Object.keys(menuData).map((menu) => {
          const color = menuData[menu].color;
      ...
      <div
        className={`mt-6 rounded-[2rem] border bg-gradient-to-br p-5 shadow-[0_20px_60px_rgba(15,23,42,0.08)] md:p-8 ${activeTheme.panel}`}
      >
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
              Select a module to open the corresponding workflow with a cleaner, more official dashboard experience.
            </p>
          </div>
          <div className="rounded-2xl border border-white/80 bg-white/80 px-4 py-3 shadow-sm backdrop-blur">
            ...
            <p className="mt-1 text-lg font-bold text-slate-900">
              {activeCategory.items.length} Modules
            </p>
          </div>
        </div>
            ...
            >
              ...
              <div className="relative flex min-h-[120px] items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className={`text-xs font-semibold uppercase tracking-[0.22em] ${activeTheme.accentText}`}>
                      Module
                    </p>
                    <span className={`${color.text} mt-2 block text-base font-bold text-slate-900 md:text-[1.05rem]`}>
                      {item.name}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-end gap-3">
                  <span className="rounded-full border border-white/80 bg-white/75 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-slate-600 shadow-sm">
                    Open
                  </span>
                  <ArrowRight
                    size={20}
                    className={`${activeTheme.arrow} transition duration-300 group-hover:translate-x-1.5`}
                  />
                </div>
              </div>
            </NavLink>
